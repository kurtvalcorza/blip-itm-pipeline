"""Static release-asset validation for the BLIP image-text matching DIMER pipeline.
...
EXPECTED_OUTPUTS = (
    ...
    "outputs/blip_itm_annotated.png",
)
...
CODE_MARKERS = (
    ...
    "correct_text_per_image = list(range(len(samples)))",
    "hashlib.sha256(np.asarray(image.convert('RGB')).tobytes()).hexdigest()",
    "result['itm_probability']",
    "result['cosine']",
...
MARKDOWN_MARKERS = (
    "**Capability:** Image-text matching and retrieval",
    "**No adaptation occurs:**",
    "**Weight-format note:**",
    "The caption set is a **caller-owned request parameter**",
    "**Neither score is calibrated and neither abstains**",
    "recall@k needs a captioned image set",
    "the verdict is `not-measurable`",
    "`sample-sanity`",
    "**The caption set is part of the request**",
    "Captioning or question answering (separate checkpoints)",
)
